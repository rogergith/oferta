...
<%@page import="com.weavx.web.model.RestrictedUser" %>
...
<spring:url value="/resources/core/js/restrictEvent.js"
	var="restrictEventJs" />
<spring:url
...
<spring:url value="/resources/core/js/paymentStripe.js"
	var="paymentStripeJs" />
	<spring:url value="/resources/core/js/listRestrictedUsers.js"
	var="usersListJs" />
<spring:url value="/resources/core/css_old/call-center.css"
	var="callCenterCSS" />
<spring:url value="/resources/core/css_old/restricted-styles.css"
	var="restrictedStyle" />
...
<link rel="stylesheet" href="${restrictedStyle}">
</head>
			...
			<div class="layout-content-body ">
				<div class="title-bar">
					<h1 class="title-bar-title">
						<span class="d-ib">Restricted Users</span> <span class="d-ib">
				...
				<!-- TABLE -->
					<table id="restricted_users" class="display">
					    <thead>
					        <tr>
					        	<th>Id</th>
					            <th>Email</th>
					    ...
					    <tbody>
					        ...
					        <%
	ArrayList<RestrictedUser> users = (ArrayList<RestrictedUser>) session.getAttribute("restrictedUsers");
	Iterator<RestrictedUser> iterListUser = users.iterator();
	while (iterListUser.hasNext()) {
		RestrictedUser userTmp = iterListUser.next();
		
%>
							<tr>
					            <td><%=userTmp.getId()%></td>
					            <td >
									<input id="mail-<%=userTmp.getId()%>" type="hidden" value="<%=userTmp.getEmail()%>">
									<%=userTmp.getEmail()%>
								</td>
					            <td >
									...
									<%--<input type="checkbox" id="on-off-switch" name="switch1" checked>--%>
										<label class="switch switch-info">
											<input class="switch-input" type="checkbox" id="active-<%=userTmp.getId()%>" <c:if test="<%=userTmp.isActive()%>">checked</c:if> onchange="changeStatusActive(<%=userTmp.getId()%>)" />
					             ...
					             <td>
					             	<div class="action-btn-container">
						            	<div class="btn-container" >
							            	<button data-id=<%=userTmp.getId()%> data-email=<%=userTmp.getEmail()%> data-active=<%=userTmp.isActive()%> class="btn btn-sm btn-primary editBtn" data-toggle="modal" data-target="#myModal">
							            		<span class="fa fa-pencil"></span>Edit
							            	</button>
						            	</div>
						            	<div class="btn-container">
						            		<button data-id=<%=userTmp.getId()%> class="btn btn-sm btn-danger deleteBtn" data-toggle="modal" data-target="#myModal">
